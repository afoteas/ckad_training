...
In this lesson, the taint is:

```text
dedicated=finance:NoSchedule
```

| Part | Value in this demo | Purpose |
|------|--------------------|---------|
...
This exercise uses `NoSchedule`, which is the most common effect for dedicated workloads.

## Toleration Matching

For a Pod to schedule on a tainted node, it needs a toleration in `spec.tolerations` that matches the taint.

The tolerating Pod in this lesson uses:
